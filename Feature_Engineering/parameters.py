...
def add_buzzer_params(parser):
    ...
    parser.add_argument('--buzzer_history_length', type=int, default=15, help="How many time steps to retain guesser history")
    parser.add_argument('--buzzer_history_depth', type=int, default=15, help="How many old guesses per time step to keep")    
...
def load_buzzer(flags, guesser_params, load=False):
    ...
    features=["Length","LG","Dict","Sr","Guesses"]
    # features=[]
    
    print("Initializing features: %s" % str(features))
    ...
    for ff in features:
        ...
        if ff == "Freq":
            from features import ModFrequencyFeature
            feature = ModFrequencyFeature(ff)
            feature.add_training()
            buzzer.add_feature(feature)
        
        if ff == "Guesses":
            from features import GuessHistoryFeature
            feature = GuessHistoryFeature(ff)
            buzzer.add_feature(feature)
        
        if ff == "LG":
            from features import LastGuessFeature
            feature = LastGuessFeature(ff)
            buzzer.add_feature(feature)
            
        if ff == "POS":
            from features import POSFeature
            feature = POSFeature(ff)
            buzzer.add_feature(feature)

        if ff == "Dict":
            from features import DictionaryFeature
            feature = DictionaryFeature(ff)
            buzzer.add_feature(feature)

        if ff == "Sr":
            from features import SearchFeature
            feature = SearchFeature(ff)
            buzzer.add_feature(feature)
            
        features_added.add(ff)

    if len(features) != len(features_added):
        ...
        logging.error(error_message % (len(features), str(features),
    ...
    def __init__(self, customized_params=None):
        ...
        if customized_params:
            ...
            dan_params = [("embed_dim", int, 300, "How many dimensions in word embedding layer"),
                          ...
                          ("max_classes", int, 100, "Maximum number of classes"),
